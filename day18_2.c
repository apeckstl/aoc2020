...
long long evaluateLine(FILE *fp)
{
    ...
    for (int i = 0; i < charactersRead; i++)
    {
        ...
        {
            // 1 While the operator stack is not empty and the top of the operator stack has the same precedence as thisOp or higher,
            while (operatorStack != NULL && precedence(operatorStack->data) >= precedence(nextToken))
            {
                // 1 Pop the operator from the operator stack.
                char operator= pop(&operatorStack);
                long long operand2 = pop(&valueStack);
