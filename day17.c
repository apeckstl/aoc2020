...
int shouldBeAdded(int,int,int, hashtable*);
...
void cycle(hashtable *ht) {
			...
			for (int i = current->x - 1; i <= current->x + 1; i++) {
				for (int j = current->y - 1; j <= current->y + 1; j++) {
					for (int k = current->z - 1; k <= current->z + 1; k++) {
						if (current->x == i && current->y == j && current->z == k)
							continue;
						if (ht_get(ht, i, j, k, 0)) {
							// neighbor is active
							neighbors++;
						} else {
							// neighbor is inactive, should we add it?
							if (shouldBeAdded(i, j, k,ht)) {
	...
	while (toBeAdded) {
		...
		ht_put(ht, toBeAdded->x, toBeAdded->y, toBeAdded->z, 0);
		free(toBeAdded);
		toBeAdded = c;
	}
	...
	while (toBeDeleted) {
		...
		ht_del(ht, toBeDeleted->x, toBeDeleted->y, toBeDeleted->z, 0);
		free(toBeDeleted);
		toBeDeleted = c;
	}
...
int shouldBeAdded(int x, int y, int z, hashtable *ht) {
	int neighbors = 0;
	for (int i = x - 1; i <= x + 1; i++) {
		for (int j = y - 1; j <= y + 1; j++) {
			for (int k = z - 1; k <= z + 1; k++) {
				if (!(x == i && y == j && z == k) && ht_get(ht, i, j, k, 0) == 1) {
					//if (x == 0 && y == 0 && z == 0)
